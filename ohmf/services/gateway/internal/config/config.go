...
type Config struct {
	...
	WSConnectPerIPLimit       int
	WSConnectBurst            int
	SendRateWindow            time.Duration
	SendRatePerUser           int
	SendRateUserBurst         int
	SendRatePerConversation   int
	SendRateConversationBurst int
	SendRatePerIP             int
	SendRateIPBurst           int
...
func Load() Config {
	return Config{
		...
		WSConnectWindow:           time.Duration(getInt("APP_WS_CONNECT_WINDOW_SECONDS", 60)) * time.Second,
		WSConnectPerIPLimit:       getInt("APP_WS_CONNECT_PER_IP_LIMIT", 60),
		WSConnectBurst:            getInt("APP_WS_CONNECT_BURST", 120),
		SendRateWindow:            time.Duration(getInt("APP_SEND_RATE_WINDOW_SECONDS", 10)) * time.Second,
		SendRatePerUser:           getInt("APP_SEND_RATE_PER_USER", 30),
		SendRateUserBurst:         getInt("APP_SEND_RATE_USER_BURST", 60),
		SendRatePerConversation:   getInt("APP_SEND_RATE_PER_CONVERSATION", 300),
		SendRateConversationBurst: getInt("APP_SEND_RATE_CONVERSATION_BURST", 500),
		SendRatePerIP:             getInt("APP_SEND_RATE_PER_IP", 120),
		SendRateIPBurst:           getInt("APP_SEND_RATE_IP_BURST", 240),

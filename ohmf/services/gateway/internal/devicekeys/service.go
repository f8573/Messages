...
func (s *Service) PublishBundle(ctx context.Context, actorUserID, deviceID string, req PublishRequest) (Bundle, error) {
	if err := s.ensureDeviceOwnership(ctx, actorUserID, deviceID); err != nil {
		return Bundle{}, err
	}
	if err := s.ensureDeviceCapability(ctx, actorUserID, deviceID, RequiredDeviceCapability); err != nil {
		return Bundle{}, err
	}
	req = normalizePublishRequest(req)
	var priorFingerprint string
	_ = s.db.QueryRow(ctx, `SELECT COALESCE(fingerprint, '') FROM device_identity_keys WHERE device_id = $1::uuid`, deviceID).Scan(&priorFingerprint)
	...
	if len(req.OneTimePrekeys) > 0 {
		if err := s.insertPrekeysTx(ctx, tx, deviceID, req.OneTimePrekeys); err != nil {
			return Bundle{}, err
		}
	}
	...
	return s.loadBundle(ctx, actorUserID, deviceID)
}
...
func (s *Service) AddOneTimePrekeys(ctx context.Context, actorUserID, deviceID string, prekeys []OneTimePrekey) (Bundle, error) {
	...
	if err := s.insertPrekeysTx(ctx, tx, deviceID, prekeys); err != nil {
		return Bundle{}, err
	}
	if err := tx.Commit(ctx); err != nil {
		return Bundle{}, err
...
func (s *Service) ensureDeviceOwnership(ctx context.Context, actorUserID, deviceID string) error {
	var exists bool
	if err := s.db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM devices WHERE id = $1::uuid AND user_id = $2::uuid)`, deviceID, actorUserID).Scan(&exists); err != nil {
		return err
	}
	if !exists {
...
func (s *Service) ensureDeviceCapability(ctx context.Context, actorUserID, deviceID, capability string) error {
	var exists bool
...
func (s *Service) insertPrekeysTx(ctx context.Context, tx pgx.Tx, deviceID string, prekeys []OneTimePrekey) error {
...
func (s *Service) loadBundle(ctx context.Context, actorUserID, deviceID string) (Bundle, error) {
	if err := s.ensureDeviceOwnership(ctx, actorUserID, deviceID); err != nil {
		return Bundle{}, err
	}
	var bundle Bundle

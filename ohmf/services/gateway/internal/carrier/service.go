...
// RowScanner is the minimal result row interface used by queries.
type RowScanner interface {
	Scan(dest ...any) error
}

// Rows is the minimal rows iterator interface used by queries.
type Rows interface {
	Next() bool
	Scan(dest ...any) error
	Close()
...
// DB abstracts the database operations used by the carrier service. An
// adapter for *pgxpool.Pool is provided at wiring sites; tests can provide
// a mock pool that satisfies this interface.
type DB interface{
